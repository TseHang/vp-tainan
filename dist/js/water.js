...
river();
function reservoir() {
  var reservoirCsv = null;
  var reservoirMap;
  var reservoirMapClick = false;
  var reservoirInfo = L.control({ position: 'topleft' });
  var reservoirlegend = L.control({ position: 'bottomleft' });
  ...
  $.ajax({
    ...
    success: function success(data) {
      reservoirCsv = $.csv.toObjects(data);
    }
  });
  ...
  function getColor(d) {
    return d > 50 ? '#2BF888' : d > 40 ? '#2BAD88' : d !== d === false ? '#288' : '#333';
  }
  // var width = 400, height = 400;

  // var reservoirSVG = d3.select("#reservoir").append("svg")
  //     .attr("width", width)
  //     .attr("height", height)
  //     .append("g")
  //     .attr("transform", "translate(0,0)");

  // var proj = d3.geo.mercator().center([121, 23.5]).scale(10000)

  // var path = d3.geo.path().projection(proj);


  // d3.json('data/twCounty2010merge.topo.json', function(json2) {
  // sueikuSvg.selectAll("path")
  //       .data(topojson.feature(json2, json2.objects.layer1, function(a) {return }).features)
  //       .enter()
  //       .append("path")
  //       .attr("stroke","#000")
  //       .attr("stroke-width",1)
  ...
  // d3.json('data/ressub.topo.json', function(json) {

  //   reservoirSVG.selectAll("path")
  //       .data(topojson.feature(json, json.objects['ressub']).features)
  //       .enter()
  //       .append("path")
  //       .attr("stroke","#000")
  //       .attr("stroke-width",1)
    ...
    $.ajax({
      ...
      success: function success(data) {
        ...
        for (var i in suppleArea) {
          ...
          var marker = L.marker(L.latLng(suppleArea[i]['lat'], suppleArea[i]['lng']), { icon: readMarker, town: suppleArea[i]['轄區'].split("、") }).addTo(siteMap);
...
function river() {
    ...
    $.getJSON('./src/data/siteInfo.json', function (data) {
      ...
      for (var i in siteInfo) {
        if (siteInfo[i].SiteName in riverData) {
          ...
          var icon = L.ExtraMarkers.icon({ prefix: 'fa', icon: 'map-marker',
  ...
  RiverInfo.update = function (props) {
    if (props && props.name + '流域' in basinRPI) {
      this._div.innerHTML = '<h4>河川流域名稱：' + (props ? props.name + '</h4>平均污染指數RPI<sup class="rpi-q"><a href="#" onclick="showRPIInfo()">[?]</a></sup>：' + basinRPI[props.name + '流域'].RPI.toFixed(1) : '</h4>請點選畫面區塊');
    } else {
