...
L.Control.geocoder({
  ...
  name = e.geocode.name
  lResult = leafletPip.pointInLayer(latlng, lLayer, true)
  if (isLoadSensitiveData)
    sResult = leafletPip.pointInLayer(latlng, sLayer, true)
  w600Result = leafletPip.pointInLayer(latlng, w600Layer, true)
  w200Result = leafletPip.pointInLayer(latlng, w200Layer, true)
  countryW600Result = leafletPip.pointInLayer(latlng, countryW600Layer, true)
  countryW200Result = leafletPip.pointInLayer(latlng, countryW200Layer, true)
  console.log("lLayer", lResult)
  if (isLoadSensitiveData)
    console.log("sLayer", sResult)
  console.log("wLayer", w600Result)
  lresponse = lResult.length > 0 ? lResult[0].feature.properties['分級'] : "無潛勢"
  if (isLoadSensitiveData)
...
info.onAdd = function(map) {
  this._div = L.DomUtil.create('table', 'ui table'); // create a div with a class "info"
  this.update();
  return this._div;
};
